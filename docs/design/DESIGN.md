...
**배지는 테두리로 세 등급을 만든다.** 값이 있으면 회색 면(`fact-badge`), 값이 낡았으면 실선 테두리에 채움 없음(`stale-badge`), 값이 없으면 점선 테두리에 채움 없음(`no-data-badge`). 색을 쓰지 않고 테두리만 바꾸는 이유는, 이 셋이 **한 카드 안에 나란히 서기 때문**이다 — 색으로 갈라 놓으면 카드마다 신호등이 하나씩 켜지고, 그러면 카드끼리 비교하게 된다.
...
구성 순서가 고정이다: **사진 → 이름(2줄) → 주소(1줄) → 배지 한 줄 → 장소 발견 신호 세 줄 → 기준 시점 캡션.** 캡션이 마지막이고 빠질 수 없다.

- **장소 발견 신호 세 줄:** 온라인 언급량 · 시·군 내 TMAP 검색순위 · 입장객 수가 **각각 제 줄**에 선다. 값이 있으면 `온라인 언급 374,649건 · 9월 기준`처럼 사실과 기준 시점을 한 줄에 적고, 없으면 그 줄에 `no-data-badge`가 온다. 셋을 한 줄로 합치거나 하나로 요약하지 마라 — 범위도 단위도 기준 시점도 달라서, 합치는 순간 없는 정밀도가 생긴다.
- **순서를 바꾸지 않는다.** 정렬이 쓰는 신호(온라인 언급량)가 먼저 오고 보조 근거가 뒤를 받친다. 값이 있는 것을 위로 끌어올리지도 않는다 — 카드마다 줄 순서가 달라지면 훑을 수 없다.

- **pressed:** 배경이 `#F9FAFB`로. 터치 기기에는 hover가 없으므로 **hover 스타일을 따로 만들지 않는다.** 데스크톱에서는 pressed와 같은 값을 hover에 쓴다.
- **저장 상태:** 오른쪽 위 `save-button`. 저장되면 `#E1EEFF` 면에 `#013B84` 아이콘.
- **캡션:** 기준 시점 한 줄뿐이다. **공급자 이름을 적지 않는다** — `한국관광공사 KorService2 ·` 같은 접두어는 카드마다 같은 말을 반복하면서 정작 사용자가 쓸 기준 시점을 밀어낸다. 출처는 도움말 한 곳에 모은다.
...
| 신호 | 없을 때의 문구 |
|---|---|
| 온라인 언급량 | `언급 판정 보류`(이름이 모호해 그 장소의 값으로 볼 수 없음) · `언급 집계 대상 아님`(검색어를 만들 수 없음) · `언급 미수집`(값을 얻지 못함) |
| TMAP 검색순위 | `TMAP 미수록` — 수록되지 않은 것이지 순위가 낮은 것이 아니다 |
| 입장객 수 | `입장객 미집계`(그 공표월에 집계 없음) · `입장객 정보 없음`(통계를 아직 확인하지 못함) |
| 방문 혼잡도 예측 | `예측 정보 없음` |
| 현재 접근 혼잡 | `현재 접근 정보 없음` |
| 중심관광지 순위 | `중심관광지 순위 미산정` |
...
- **카드에서는 배지**(`최근 저장된 예측`), **캡션 자리에서는 한 줄**(`최근 저장된 정보 · 3시간 전 기준`)이다. 얼마나 지난 값인지 함께 적고, 기준 시점을 모르면 `최근 저장된 정보`까지만 적는다 — 모르는 것을 `0시간 전`으로 만들지 않는다.
...
- **램프 3단과 4단이 가깝다(주의).** 새 포인트 색에서 `primary`(L 33.3%)와 `primary-strong`(L 26.1%)의 명도 차가 7.3%p로, 옛 값(9.8%p)보다 좁아졌다. 인접 대비 1.33:1. 범례가 항상 켜져 있어 구간 판독은 범례가 받치지만, 지도만 보고 3단과 4단을 가르기는 전보다 어렵다 — 실사용에서 문제가 되면 램프 4단을 포인트 색과 분리해 따로 잡는다(이 문서가 아니라 1단계 재실행 사안).
