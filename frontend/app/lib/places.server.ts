...
function toListResponse(data: BackendListResponse, search: PlaceListSearch | null): PlaceListResponse {
  return {
...
export async function fetchPlaceList(
  ...
  if (term) {
    ...
    return result.ok
      ? { ok: true, data: toListResponse(result.data, toSearchInfo(result.data)) }
  ...
  if (result.ok) {
    return { ok: true, data: toListResponse(result.data, null) };
  }
  ...
  return retried.ok
    ? { ok: true, data: toListResponse(retried.data, null) }
