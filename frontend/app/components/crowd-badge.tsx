...
/**
 * 예측이 없다고 말하는 배지. **상세 화면 전용이다** (#49).
 *
 * 규칙이 자리마다 다른 것이 아니라, 빈자리가 무슨 뜻으로 읽히는지가 다르다. 상세는
 * 한 장소의 근거를 늘어놓는 자리라서 배지가 빠지면 그 줄 자체가 사라지고, 빈자리가
 * `예측이 좋다`로 읽힌다 — 그래서 여기서는 숨기지 않는다. 목록 카드는 날짜 조건이
 * 걸렸을 때만 예측 배지 하나를 갖고, 그 하나가 없으면 카드에 예측 이야기가 아예
 * 없으므로 좋게도 나쁘게도 읽히지 않는다 (`card-facts`).
 */
